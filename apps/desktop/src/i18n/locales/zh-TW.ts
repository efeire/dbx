...
export default withEnglishFallback({
  ...
  grid: {
    ...
    serverExecuteTimeHint: "OceanBase SQL Audit EXECUTE_TIME 表示伺服器執行計畫的耗時。JDBC 游標顯示原陳述式結束後的樣本；獨立分頁 SQL 僅在各頁皆有樣本時顯示總和。與執行耗時和等待耗時的範圍重疊。游標尚未結束，或無法讀取、關聯稽核資料時無法取得。",
    agentExecuteTimeHint: "從設定 schema 到 JDBC execute 傳回的 Agent 耗時；不含讀取資料列，並非資料庫伺服器的純執行時間。",
    clientRequestWait: "等待耗時：{ms} 毫秒",
    clientRequestWaitHint: "桌面端送出查詢請求至收到完整結果的耗時，包含讀取資料列、可用時的伺服器稽核查詢和傳輸，不含 SQL 準備與表格繪製；載入多頁時累加各頁等待時間。",
    resultViewUpdate: "表格更新：{ms} 毫秒",
    resultViewUpdateHint: "從表格收到本次結果，到 DOM 更新完成或 Canvas 繪製提交；不含 GPU 合成及螢幕實際呈現。",
    rows: "{count} 列",
    totalRows: "共 {count} 筆",
